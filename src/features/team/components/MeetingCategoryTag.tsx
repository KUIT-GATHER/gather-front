...
const TAG_STYLE: Record<PostingCategory, { idle: string; selected: string }> = {
  ENVIRONMENT: {
...
const TAG_ICON: Record<PostingCategory, string> = {
...
type MeetingCategoryTagProps = {
  category: PostingCategory;
  selected: boolean;
};

export function MeetingCategoryTag({
  category,
  selected,
}: MeetingCategoryTagProps) {
  return (
    <span
      className={cn(
        ...
        selected ? TAG_STYLE[category].selected : TAG_STYLE[category].idle,
        selected ? "text-[#fafaf8]" : "text-[#5e5e5d]",
      )}
    >
      <img
        src={TAG_ICON[category]}
        alt=""
        aria-hidden="true"
        className={cn(
          "size-[18px]",
